...
from forge.ops import ForgeFusedMoEFunction
...
class ForgeBlockSparseTop2MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, x):
        return self.w2(ForgeSiLUMulFunction.apply(self.w1(x), self.w3(x)))


class ForgeExperts(nn.Module):
    """
    Patch MixtralExperts for transformers v5 or later to use ForgeSiLUMulFunction
    ...
    def forward(self, hidden_states, top_k_index, top_k_weights):
        ...
        out = ForgeFusedMoEFunction.apply(x, self.gate_up_proj, self.down_proj, top_k_index_2d, top_k_weights_2d)
        return out.view(orig_shape)


class ForgePhi3SwiGLUMLP(nn.Module):
    """
    Patch Phi3MLP to use ForgeSiLUMulFunction
    ...
    def forward(self, x):
        ...
        return self.down_proj(ForgeSiLUMulFunction.apply(gate, up_states))


class ForgeQwen3MoeSwiGLUMLP(nn.Module):
    """
    Patch Qwen3MoeMLP to use ForgeSiLUMulFunction.
...
class ForgeHunyuanV1SwiGLUMLP(nn.Module):
...
class ForgeFalconH1SwiGLUMLP(nn.Module):
    """
    Patch FalconH1MLP to use ForgeSiLUMulFunction with gate / down multipliers.
